...
def user2cookie(user, max_age):
    ...
    sh = '%s-%s-%s-%s' % (user.id, user.passwd, expires, _COOKIE_KEY)
    L = [user.id, expires, hashlib.sha1(sh.encode('utf-8')).hexdigest()]
    return '-'.join(L)
...
async def cookie2user(cookie_str):
    ...
    try:
        ...
        if user is None:
            return None
        sh = '%s-%s-%s-%s' % (user.id, user.passwd, expires, _COOKIE_KEY)
        if sha1 != hashlib.sha1(sh.encode('utf-8')).hexdigest():
...
def signout(request):
    ...
    r.set_cookie(COOKIE_NAME, '__deleted__', max_age=0, httponly=True)
...
async def api_register_user(*, email, name, passwd):
    ...
    if not email or not _RE_EMAIL.match(email):
        raise APIValueError('email', 'email cannot be empty or invalid')
    if not passwd or not _RE_SHA1.match(passwd):
        raise APIValueError('passwd', 'password cannot be empty or invalid')
    uid = next_id()
